...
const HomeScreen: React.FC = () => {
  ...
  const { entries, isLoading: contextIsLoading, plan } = useAppContext(); // Removed isLimitReached as it's derived
  ...
  return (
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <FlatList
        ...
        contentContainerStyle={filteredAndSortedEntries.length === 0 ? styles.centeredContent : styles.listContent}
      ...
      <FAB
        ...
        style={[
            styles.fab,
            // Fix: Color properties will be valid after theme.ts fixes.
            { backgroundColor: canAddMoreEntries ? theme.colors.primary : theme.colors.surfaceDisabled },
        ]}
        // Fix: Color properties will be valid after theme.ts fixes.
        color={canAddMoreEntries ? theme.colors.onPrimary : theme.colors.onSurfaceDisabled}
        onPress={() => {
        ...
        visible={true}
        accessibilityLabel={canAddMoreEntries ? "Add new entry" : "Upgrade to add more entries"}
      />
    </View>
  );
};
